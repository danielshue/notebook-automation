...
from tools.utils.config import setup_logging, ONEDRIVE_LOCAL_RESOURCES_ROOT, VAULT_LOCAL_ROOT
...
def move_txt_to_onedrive(txt_path, video_path):
    ...
    try:
        ...
        if not os.path.exists(video_path.parent):
            ...
            try:
                from tools.utils.config import ONEDRIVE_LOCAL_RESOURCES_ROOT
...
def process_transcript(transcript_path, video_path, dry_run=False):
    ...
    try:
        ...
        cloud_path = None
        from tools.onedrive.sharing import convert_local_path_to_cloud_path

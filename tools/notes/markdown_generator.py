...
def normalize_path(path):
    """
    Normalize a file path to work in both Windows and WSL/Linux environments.
    Converts Windows paths (C:\...) to WSL paths (/mnt/c/...).
    ...
    path = path.replace('\\', '/')
    # Convert Windows drive letter to /mnt/<drive>
...
"""
    ...
    # Format the local file path for linking (absolute path, no duplication)
    local_file_path = f"file://{video_path.resolve()}"
    ...
    formatted_template = video_template.format(
        ...
        local_file_path=local_file_path,  # Only in YAML, not in markdown body
    ...
    # Remove Local File Path from markdown body if present
    formatted_template = re.sub(r'\[📁 Local File Path\]\([^)]+\)\n?', '', formatted_template)
    # Ensure no YAML is placed after the Local File Path or anywhere in the body
    formatted_template = re.sub(r'---\n.*---\n', '', formatted_template, count=1, flags=re.DOTALL) + '\n'
    ...
    try:
        ...
        if transcript_text and not (args and getattr(args, 'no_summary', False)):
            if OPENAI_API_KEY:
                ...
                # Generate summary
                ai_summary = generate_summary_with_openai(transcript_text, video_name, course_name)

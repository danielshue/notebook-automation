 ...
 */
export function getRelativeVaultResourcePath(
  fullPath: string,
  vaultRoot: string,
  vaultBase?: string,
): string {
...
export function getNaExecutableName(): string {
  ...
  if (
    archString === "x64" ||
    archString === "x86_64" ||
    (archString.includes("64") && !archString.includes("arm"))
  ) {
    archName = "x64";
  } else if (
    archString === "arm64" ||
    archString === "aarch64" ||
    archString.includes("arm")
  ) {
    archName = "arm64";
  } else {
    archName = "x64";
  ...
  try {
    ...
    const tryFindExecutable = (dir: string): string | null => {
      ...
      try {
        ...
        const naExecutables = files.filter(
          (file: string) =>
    ...
    const isValidPluginDir = (
      ...
      const isAbsolute = path
        ? path.isAbsolute(dir)
        : dir.startsWith("/") || dir.match(/^[A-Za-z]:/);
      if (!isAbsolute && !vaultRoot) return false;
      return true;
    };
    if (
      plugin.manifest &&
      isValidPluginDir(plugin.manifest.dir, plugin.manifest.id) &&
      path
    ) {
      let resolvedDir = plugin.manifest.dir || "";
      if (
        resolvedDir &&
        (resolvedDir.startsWith("/.obsidian") ||
          resolvedDir.startsWith("/.") ||
          (resolvedDir.startsWith("/") && !fs?.existsSync?.(resolvedDir)))
      ) {
        resolvedDir = resolvedDir.substring(1);
      }
      const isAbsolute =
        path.isAbsolute(resolvedDir) && fs?.existsSync?.(resolvedDir);
    ...
    if (vaultRoot && plugin.manifest?.id) {
      ...
      const lastResortDir = path
        ? path.join(vaultRoot, configDir, "plugins", pluginId)
        : "";
      if (lastResortDir) {
 ...
 */
export async function ensureExecutableExists(
  plugin: Plugin,
  ...
  try {
    checksumMap = await getOrDownloadChecksums(plugin);
  } catch {
    /* ignore */
  }
  try {
    ...
    if (fs && fs.existsSync(actualFilePath)) {
      ...
      const versionOk = await isExecutableCurrent(
        plugin,
        actualFilePath,
        expectedVersion,
      );
      ...
      if (versionOk && checksumMap && checksumMap[execName]) {
        checksumOk = await verifyFileChecksum(
          fs,
          actualFilePath,
          checksumMap[execName],
        );
        if (!checksumOk) {
          console.log(
            `[Notebook Automation] Executable exists with correct version but checksum mismatch - will redownload`,
          );
        } else {
          console.log(
            `[Notebook Automation] Executable exists with correct version and valid checksum - skipping download`,
          );
        }
      }
    ...
    if (needsDownload) {
      ...
      try {
        if (!checksumMap || (checksumMap && !checksumMap[execName])) {
          checksumMap = await getOrDownloadChecksums(plugin);
        }
      } catch {
        /* ignore */
      ...
      if (
        ...
        const verified = await verifyFileChecksum(
          fs,
          finalPath,
          checksumMap[execName],
        );
        if (!verified) {
          console.warn(
            "[Notebook Automation] Executable checksum mismatch after download - file may be corrupted. Attempting re-download...",
          );
          ...
          const redownloaded = await downloadExecutableFromGitHub(
            plugin,
            progressCallback,
          );
          if (checksumMap[execName]) {
            const recheck = await verifyFileChecksum(
              fs,
              redownloaded,
              checksumMap[execName],
            );
            if (!recheck) {
              console.error(
                "[Notebook Automation] Executable checksum mismatch after re-download.",
              );
              console.error("[Notebook Automation] This may indicate:");
              console.error(
                "[Notebook Automation]   1. Network issues during download",
              );
              console.error(
                "[Notebook Automation]   2. Incompatible executable for your platform",
              );
              console.error(
                "[Notebook Automation]   3. GitHub release assets are corrupted",
              );
              console.error(
                "[Notebook Automation] The executable may not work correctly. Try:",
              );
              console.error(
                "[Notebook Automation]   - Reload Obsidian completely",
              );
              console.error(
                "[Notebook Automation]   - Check your internet connection",
              );
              console.error(
                "[Notebook Automation]   - Report this issue on GitHub",
              );
              // Still proceed, but user has been warned extensively
            } else {
              console.log(
                "[Notebook Automation] Executable re-download successful, checksum verified.",
              );
            }
          }
          ...
          console.log(
            "[Notebook Automation] Downloaded executable checksum verified successfully.",
          );
        }
      }
    }
 ...
 * Always force-downloads to ensure checksums match the current plugin version.
 */
async function getOrDownloadChecksums(
  plugin: Plugin,
  ...
  try {
    ...
    // Force download to ensure we have checksums for the current version
    const filePath = await downloadFileFromGitHub(
      plugin,
      "checksums.json",
      false,
      undefined,
    ...
    // Validate that checksums version matches expected plugin version
    if (json.version && json.version !== expectedVersion) {
      console.warn(
        `[Notebook Automation] Checksums version mismatch: expected ${expectedVersion}, got ${json.version}. Checksums may be stale.`,
      );
    }
        ...
        const redirectRequest = https.get(
          ...
          (redirectResponse: any) => {
            if (redirectResponse.statusCode !== 200) {
              reject(
                new Error(
                  `Failed to download ${fileName}: HTTP ${redirectResponse.statusCode}`,
                ),
              );
              return;
            }
 ...
 * @throws If required Node.js modules are not available or download fails.
 */
export async function downloadExecutableFromGitHub(
  plugin: Plugin,
  progressCallback?: DownloadProgressCallback,
): Promise<string> {
  const execName = getNaExecutableName();
  try {
    ...
    return await downloadFileFromGitHub(
      plugin,
      execName,
      true,
      progressCallback,
 ...
 */
async function downloadAssetManifest(
  plugin: Plugin,
  ...
  try {
    ...
    console.log(
      `[Notebook Automation] Attempting to download asset manifest for version ${expectedVersion} from: ${expectedUrl}`,
    );

    // Force download of manifest to ensure we get the correct version
    const manifestPath = await downloadFileFromGitHub(
      plugin,
      "asset-manifest.json",
      false,
      undefined,
    ...
    if (manifest.version !== expectedVersion) {
      console.warn(
        `[Notebook Automation] Manifest version mismatch: expected ${expectedVersion}, got ${manifest.version}. Re-downloading...`,
      );
      // Delete the mismatched manifest and try once more
      try {
        fs.unlinkSync(manifestPath);
      } catch {}
      const retryPath = await downloadFileFromGitHub(
        plugin,
        "asset-manifest.json",
        false,
        undefined,
      ...
      if (retryManifest.version !== expectedVersion) {
        console.error(
          `[Notebook Automation] Manifest version still incorrect after retry. Using fallback.`,
        );
        return null;
      }
      console.log(
        `[Notebook Automation] Downloaded asset manifest for version ${retryManifest.version} with ${retryManifest.files?.length || 0} files`,
      );
      return retryManifest;
    }

    console.log(
      `[Notebook Automation] Downloaded asset manifest for version ${manifest.version} with ${manifest.files?.length || 0} files`,
    );
    return manifest;
  } catch (error) {
    const version = plugin.manifest?.version || "0.1.0-beta.2";
    ...
    console.log(
      `[Notebook Automation] Asset manifest not available for version ${version} at ${expectedUrl}, using fallback file list. This is normal for some releases.`,
    );
    return null;
  }
}
...
export async function ensureRequiredFilesExist(
  ...
  if (manifest && manifest.files) {
    ...
    const coreFiles = ["main.js", "manifest.json", "styles.css"];
    filesToDownload = manifest.files.filter(
      (file) =>
    ...
    console.log(
      `[Notebook Automation] Using manifest-based download for ${filesToDownload.length} files`,
    );
  } else {
    // Fallback to hardcoded list if manifest unavailable
    const allFiles = getDistributedPluginFiles();
    filesToDownload = allFiles.filter(
      (file) => !["main.js", "manifest.json", "styles.css"].includes(file),
    );
  ...
  for (let i = 0; i < filesToDownload.length; i++) {
    ...
    try {
      ...
      console.warn(
        `[Notebook Automation] Could not download ${fileName}:`,
        error,
      );
...
export async function ensureConfigFilesExist(plugin: Plugin): Promise<boolean> {
  try {
    ...
    if (!fs || !path) {
      console.warn(
        "[Notebook Automation] File system modules not available for config check",
      );
      return false;
    }
    ...
    if (!pluginDir) {
      console.warn(
        "[Notebook Automation] Could not determine plugin directory for config check",
      );
      return false;
    }
    ...
    const missingFiles = configFiles.filter(
      (file) => !fs.existsSync(path.join(pluginDir, file)),
    );

    if (missingFiles.length > 0) {
      console.log(
        `[Notebook Automation] Missing config files: ${missingFiles.join(", ")}. Downloading...`,
      );
      await ensureRequiredFilesExist(plugin);

      // Verify download success
      const stillMissing = configFiles.filter(
        (file) => !fs.existsSync(path.join(pluginDir, file)),
      );
      if (stillMissing.length > 0) {
        console.warn(
          `[Notebook Automation] Still missing after download: ${stillMissing.join(", ")}`,
        );
        return false;
      }
    }
    ...
    console.warn(
      "[Notebook Automation] Error ensuring config files exist:",
      error,
    );
    return false;
  }
 ...
 */
export async function isExecutableCurrent(
  plugin: Plugin,
  execPath: string,
  ...
  try {
    ...
    try {
      ...
      console.warn(
        `[Notebook Automation] Failed to check executable version: ${execError.message}`,
      );
      console.warn(
        "[Notebook Automation] The executable may be corrupted or incompatible with your system.",
      );
      return false;
    }
    ...
    const line =
      output
        .split(/\r?\n/)
        .find((l) =>
          l.toLowerCase().startsWith("notebook automation version"),
        ) || "";
    ...
    const firstTokenMatch = line.match(
      /Notebook Automation version\s+([^\s]+)(?:\s+|$)/i,
    );
    ...
    if (window?.console) {
      ...
      console.log(
        `[Notebook Automation] Executable version mismatch. Expected semantic ${expectedPluginVersion} not found in line: "${line}". First token captured='${firstToken}'. Forcing re-download.`,
      );
    }
    return false;
  } catch {

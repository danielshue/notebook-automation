...
namespace NotebookAutomation.Core.Tests
{
    ...
    public class RecursiveCharacterTextSplitterTests
    {
        ...
        {
            // Arrange
            var splitter = new RecursiveCharacterTextSplitter(_mockLogger.Object);

            // Act
            var result = splitter.SplitText(string.Empty);
        ...
        {
            // Arrange
            var splitter = new RecursiveCharacterTextSplitter(_mockLogger.Object, 1000);
            string smallText = "This is a small text that should fit in a single chunk.";

            // Act
            var result = splitter.SplitText(smallText);
        ...
        {
            // Arrange
            var splitter = RecursiveCharacterTextSplitter.CreateForMarkdown(_mockLogger.Object, 200, 50);
            ...
            // Act
            var result = splitter.SplitText(markdown);

            // Assert

            // Find the chunk that has the code block
            var codeChunk = result.FirstOrDefault(chunk => chunk.Contains("```csharp"));
        ...
        public void SplitText_WithVeryLargeText_CreatesMultipleChunks()
        {
            // Arrange
            int maxTokens = 100;
            var splitter = new RecursiveCharacterTextSplitter(_mockLogger.Object, maxTokens, 20);
            ...
            // Act
            var result = splitter.SplitText(largeText.ToString());

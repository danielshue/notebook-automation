    ...
    {        /// <summary>
        ...
        public void Register(RootCommand rootCommand, Option<string> configOption, Option<bool> debugOption, Option<bool> verboseOption, Option<bool> dryRunOption)
        {
            ...
            videoCommand.SetHandler(async context =>
            {
                ...
                AnsiConsoleHelper.WriteInfo($"  Timeout: {(timeout.HasValue ? timeout.Value.ToString() : "(default)")}"); AnsiConsoleHelper.WriteInfo($"  Resources root: {resourcesRoot ?? "(default)"}");
                ...
                logger.LogInformation("Output will be written to: {OutputPath}",
                    overrideOutputDir ?? appConfig.Paths?.ObsidianVaultRoot ?? "Generated"); var result = await batchProcessor.ProcessVideosAsync(

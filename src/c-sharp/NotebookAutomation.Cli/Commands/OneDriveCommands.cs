...
internal class OneDriveCommands
{
    ...
    {
        this.logger = logger;
        this.logger.LogDebug("OneDrive command initialized");
    }
    ...
    public void Register(RootCommand rootCommand, Option<string> configOption, Option<bool> debugOption, Option<bool> verboseOption, Option<bool> dryRunOption)
    {
        ...
        listCommand.SetHandler(async context =>
        {
            ...
            await ExecuteOneDriveCommandAsync("list", path, null, config, debug, verbose, dryRun).ConfigureAwait(false);
        });
        ...
        downloadCommand.SetHandler(async context =>
        {
            ...
            await ExecuteOneDriveCommandAsync("download", remotePath, localPath, config, debug, verbose, dryRun).ConfigureAwait(false);
        });            // Upload command
        ...
        uploadCommand.SetHandler(async context =>
        {
            ...
            await ExecuteOneDriveCommandAsync("upload", localPath, remotePath, config, debug, verbose, dryRun).ConfigureAwait(false);
        });            // Search command
        ...
        searchCommand.SetHandler(async context =>
        {
            ...
            await ExecuteOneDriveCommandAsync("search", query, null, config, debug, verbose, dryRun).ConfigureAwait(false);
        ...
        params (string Key, string Value)[] extraOptions)
    {
        try
        {
            ...
            logger.LogInformation($"Executing OneDrive command: {command}");
            ...
            {
                case "list":
                    logger.LogInformation($"Listing files at path: {arg1 ?? "root"}");
                ...
                case "download":
                    ...
                    {
                        logger.LogInformation($"Downloading from {arg1} to {arg2}");
                    }
                ...
                case "upload":
                    ...
                    {
                        logger.LogInformation($"Uploading from {arg1} to {arg2}");
                    }
                ...
                case "search":
                    logger.LogInformation($"Searching for: {arg1 ?? string.Empty}");
                ...
                case "sync":
                    ...
                    logger.LogInformation($"Syncing between {arg1 ?? string.Empty} and {arg2 ?? string.Empty} (direction: {direction})");
    ...
    public async Task ExecuteOneDriveCommandAsync(string command, string? arg1, string? arg2, string? config, bool debug, bool verbose, bool dryRun)
    {
        ...
        logger.LogDebug("Debugging OneDrive command");
        ...
        switch (command.ToLower())
        {
            case "list":
                ...
                {
                    logger.LogInformation($"Listing files at path: {arg1}");
                }
            ...
            case "search":
                if (!string.IsNullOrEmpty(arg1))
                {
                    logger.LogInformation($"Searching for: {arg1}");
                }
            ...
            case "sync":
                ...
                {
                    logger.LogInformation($"Syncing between {arg1} and {arg2}");
                }

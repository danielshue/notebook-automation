...
/// <summary>
/// Service for markdown generation operations.
/// </summary>
/// <remarks>
/// This service wraps <see cref="MarkdownNoteBatchProcessor"/> to provide a unified API for Copilot tools.
/// </remarks>
/// <param name="logger">The logger instance.</param>
/// <param name="markdownBatchProcessor">The markdown batch processor.</param>
/// <param name="appConfig">The application configuration.</param>
/// <param name="userSecrets">The user secrets helper for API keys.</param>
public class MarkdownService(
    ILogger<MarkdownService> logger,
    MarkdownNoteBatchProcessor markdownBatchProcessor,
    AppConfig appConfig,
    UserSecretsHelper userSecrets) : IMarkdownService
{
    private readonly ILogger<MarkdownService> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    private readonly MarkdownNoteBatchProcessor _markdownBatchProcessor = markdownBatchProcessor ?? throw new ArgumentNullException(nameof(markdownBatchProcessor));
    ...
    /// <inheritdoc />
    public async Task<MarkdownOperationResult> GenerateAsync(
        string inputPath,
    ...
    {
        _logger.LogInformation("Generating markdown from path: {Path}", inputPath);
        ...
        try
        {
            ...
            var htmlExtensions = _appConfig.HtmlExtensions.Count > 0
                ? _appConfig.HtmlExtensions
                : [".html", ".htm", ".epub"];
            ...
            var result = await _markdownBatchProcessor.ProcessFilesAsync(
                resolvedInput,
                resolvedOutput,
                htmlExtensions,
                apiKey,
            ...
            return new MarkdownOperationResult
            {
        ...
        {
            _logger.LogError(ex, "Error generating markdown from {Path}", inputPath);
    ...
    /// <summary>
    /// Gets the default output path for markdown notes.
    /// </summary>
    ...
    /// <summary>
    /// Creates an error result for Generate operation.
    /// </summary>
    private static MarkdownOperationResult CreateErrorResult(string message)
    {
        return new MarkdownOperationResult
        {
            Success = false,

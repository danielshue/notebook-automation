...
from notebook_automation.tools.utils.config import setup_logging, ONEDRIVE_LOCAL_RESOURCES_ROOT, VAULT_LOCAL_ROOT, VIDEO_EXTENSIONS
from notebook_automation.tools.utils.http_utils import create_requests_session, check_api_health
...
from notebook_automation.tools.onedrive.file_operations import (
...
from notebook_automation.tools.notes.video_markdown_generator import create_markdown_note_for_video, extract_metadata_from_path
from notebook_automation.tools.ai.summarizer import generate_summary_with_openai
from notebook_automation.tools.pdf.processor import extract_pdf_text, infer_course_and_program
from notebook_automation.tools.pdf.note_generator import create_markdown_note_for_pdf
from notebook_automation.tools.ai.summarizer import generate_summary_with_openai
...
def process_single_video(item, headers, templates, results_file='video_links_results.json', args=None):
    ...
    try:
        ...
        local_path = ONEDRIVE_LOCAL_RESOURCES_ROOT / rel_path_clean.replace('/', os.path.sep)
        # Calculate note path early to check if it exists
        video_name = local_path.stem
        note_name = f"{video_name}-video.md"
        try:
            rel_path_for_vault = local_path.relative_to(ONEDRIVE_LOCAL_RESOURCES_ROOT)
        except ValueError:
            rel_path_for_vault = Path(local_path.parts[-2]) / local_path.name
        vault_path = VAULT_LOCAL_ROOT / rel_path_for_vault
        ...
        note_path = create_markdown_note_for_video(local_path, share_link, VAULT_LOCAL_ROOT, item)
        logger.info(f"Created markdown note: {note_path}")
        print("  │  └─ Note created at: " + str(note_path).replace(str(VAULT_LOCAL_ROOT), "").lstrip('/\\'))
...
def process_videos_for_sharing(args=None):
    ...
    for index, item in enumerate(video_items):
        ...
        if item_path and '/root:/' in item_path:
            ...
            rel_path = item['name']  # Fall back to just the filename
        local_path = ONEDRIVE_LOCAL_RESOURCES_ROOT / rel_path.replace('/', os.path.sep)

...
def main():
    ...
    # Print colorized banner to console
    print(f"\n{BG_BLUE}{BOLD}{HEADER}   Notebook PDF Note Generator   {ENDC}\n")
    print(f"{OKBLUE}{BOLD}== Options =={ENDC}")
    print(f"  {OKCYAN}Debug mode         {ENDC}: {OKGREEN if args.debug else GREY}{'ON' if args.debug else 'OFF'}{ENDC}")
    print(f"  {OKCYAN}Dry run            {ENDC}: {OKGREEN if args.dry_run else GREY}{'ON' if args.dry_run else 'OFF'}{ENDC}")
    print(f"  {OKCYAN}Force overwrite    {ENDC}: {OKGREEN if args.force else GREY}{'ON' if args.force else 'OFF'}{ENDC}")
    print(f"  {OKCYAN}Generate summaries {ENDC}: {OKGREEN if not args.no_summary else GREY}{'ON' if not args.no_summary else 'OFF'}{ENDC}")
    print(f"  {OKCYAN}Create share links {ENDC}: {OKGREEN if not args.no_share_links else GREY}{'ON' if not args.no_share_links else 'OFF'}{ENDC}")
    ...
    # Remove timestamp from all handlers for logger and failed_logger
    import logging
    ...
    if not args.no_share_links:
        logger.info(f"{OKBLUE}🔐 Authenticating with Microsoft Graph API...{ENDC}")
        access_token = authenticate_graph_api(force_refresh=args.refresh_auth)
        if access_token:
            logger.info(f"{OKGREEN}  └─ Authentication successful ✓{ENDC}")
        else:
            logger.error(f"{FAIL}  └─ Authentication failed, continuing without sharing links ✗{ENDC}")
            access_token = None
    else:
        access_token = None
        logger.warning(f"{WARNING}🔐 Skipping authentication (--no-share-links is set){ENDC}")
    ...
    print(f"\n{BG_BLUE}{BOLD}{HEADER}   PDF Note Generation Summary   {ENDC}\n")
    print(f"{OKBLUE}{BOLD}== Results =={ENDC}")
    print(f"  {OKCYAN}PDFs processed      {ENDC}: {OKGREEN}{len(processed)}{ENDC}")
    print(f"  {OKCYAN}Errors encountered  {ENDC}: {FAIL if errors else OKGREEN}{len(errors)}{ENDC}")
    print(f"  {OKCYAN}Logs saved to       {ENDC}: {GREY}pdf_notes_generator.log{ENDC}")
    if errors:
        print(f"  {OKCYAN}Error details       {ENDC}: {FAIL}pdf_notes_failed.log{ENDC}")
    print()
    if processed:
        print(f"{OKGREEN}PDF note generation complete!{ENDC}")
    else:
        print(f"{WARNING}No PDFs were processed.{ENDC}")
